...
English, Hindi, and Telugu, UI chrome only (`lib/i18n/`) — nav labels, dashboards, auth screens, Settings, AI Chat, and now the full Assignments/Classes/Students/Timetable/Materials/Roster surface (including the create/generate forms and their sub-components); teacher-authored content, materials, and AI output are never translated — AI-generated content (chat replies, generated test questions) instead gets a language directive passed to Gemini via `systemInstruction` on the `generateContent` call (`/api/chat`, `/api/assignments/generate`), since a static dictionary can never cover text that's generated fresh each time. Preference is a cookie (`school-buddy-language`, `LanguageProvider`/`useLanguage()` for client components, `getServerT()` for server components — reading `localStorage` wouldn't reach the async server components the dashboards and Settings are built as), switched from a picker in **Settings**, `router.refresh()`'d so server-rendered content picks it up immediately. Not yet covered: admin-only account-management screens (`/admin/teachers`, `/admin/students`), and the Exams/Learning Videos placeholder pages.
