...
English, Hindi, and Telugu, UI chrome only — nav tabs, dashboards, auth screens, Settings, AI Chat, and every tab screen (Classes, Students, Subjects, Assignments, and the subject-offering detail screen with its inline roster/assignment form). Teacher-authored content (names, subjects, materials) and AI-generated output are never dictionary-translated — they're real data, not app chrome; AI replies instead get a language directive passed to Gemini directly (see AI Chat section in `web/README.md`).

Runs on **i18next** + **react-i18next** (`lib/i18n.ts`), reading the same `constants/i18n/*.json` files web's `messages/*.json` produce — kept in sync via `npm run i18n:sync` (root, `scripts/sync-i18n.mjs`; web is canonical) instead of a hand-run `cp`, and checked for drift/missing keys with `npm run i18n:check`. `stores/languageStore.ts` (Zustand + `AsyncStorage`, mirrors the pattern in `stores/authStore.ts`) is kept as the app's stable call-site API — every screen still does `const { t } = useLanguageStore()` exactly as before — but internally `t`/`setLanguage` now delegate to i18next, which is what actually resolves translations and drives `{param}` interpolation; Zustand+AsyncStorage still just persists which language was picked across app restarts, and pushes it into i18next on boot. See `web/README.md`'s Multi-language section for the full architecture (both platforms share the same JSON key structure) and the current gap: next-intl's ICU plural syntax works on web, but mobile would need the separate `i18next-icu` plugin to support the same syntax — not installed yet, so pluralized strings aren't used on either platform currently.
